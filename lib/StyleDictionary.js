...
import { convertToTokenMap } from './utils/convertToTokenMap.js';
...
import { convertToTokenObject } from './utils/convertToTokenObject.js';
...
export default class StyleDictionary extends Register {
  ...
  async extend(config = this.config, opts = {}) {
        ...
        function Collision(prop) {
          GroupMessages.add(
            ...
            `Collision detected at: ${prop.path.join('.')}! Original value: ${
              prop.target[/** @type {string} */ (prop.key)]
            }, New value: ${prop.copy[/** @type {string} */ (prop.key)]}`,
          );
        },
        true,
        appliedParsers,
    ...
    this.allTokens = flattenTokens(this.tokens, this.usesDtcg);
    this.tokenMap = convertToTokenMap(this.allTokens);
  ...
  async _exportPlatform(platform) {
    ...
    if (platformConfig.preprocessors && platformConfig.preprocessors.length > 0) {
      tokens = await preprocess(tokens, platformConfig.preprocessors, this.hooks.preprocessors);
      tokensChanged = true;
    }
    ...
    if (tokensChanged) {
      tokenMap = convertToTokenMap(flattenTokens(tokens, this.usesDtcg));
    }
    ...
    // TODO: use Map and convert back to array / tokens obj
    allTokens = flattenTokens(tokens, this.usesDtcg);
    tokenMap = convertToTokenMap(allTokens);
    tokens = convertToTokenObject(/** @type {Map<string,TransformedToken>} */ (tokenMap));
    return {
      tokens,
